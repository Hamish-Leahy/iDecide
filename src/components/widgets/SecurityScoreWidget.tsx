...
import { Shield, ArrowUp, AlertCircle, CheckCircle } from 'lucide-react';
  ...
  useEffect(() => {
    const loadSecurityData = async () => {
      ...
      try {
        ...
        // Load security recommendations based on user's data
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold flex items-center gap-2">
            <Shield size={18} className="text-[#1E1B4B]" />
            iDecide Score
          </h3>
        </div>
            ...
            <div className="space-y-2">
              ...
              {recommendations.map((recommendation, index) => (
                <div key={index} className="flex items-start gap-2 p-2 rounded-lg bg-[#F5F8F7]">
                  <AlertCircle size={16} className="text-[#1E1B4B] flex-shrink-0 mt-0.5" />
